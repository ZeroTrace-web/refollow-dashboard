...
(function () {
  ...
  function createRowElement(account, ctx) {
    ...
    const statusIcon = account.status === 'followed' ? '✓' : account.status === 'skipped' ? '↷' : '○';
    badge.textContent = `${statusIcon} ${statusLabel}`;
    badge.title = `Status: ${statusLabel}. Updated ${formatRelativeOrDate(account.updatedAt)}`;
  ...
  function createTableElement(account, ctx) {
    ...
    const tableStatus = node.querySelector('.account-table__status');
    const tableStatusLabel = STATUS_LABELS[account.status] || account.status;
    const tableStatusIcon = account.status === 'followed' ? '✓' : account.status === 'skipped' ? '↷' : '○';
    tableStatus.textContent = `${tableStatusIcon} ${tableStatusLabel}`;
    tableStatus.dataset.status = account.status;
    tableStatus.setAttribute('aria-label', `Status: ${tableStatusLabel}`);
