...
(function () {
  ...
  const state = {
    ...
    focusList: false,
    focusUiHidden: false,
    dashboardControlsOpen: false,
    textScale: settings.get('textScale', 'normal'),
    highContrast: settings.get('highContrast', false) === true,
    lastOpenedId: null,
    search: '',
    currentPage: 1,
    pageSize: 50,
  ...
  const VALID_TEXT_SCALES = new Set(['normal', 'large']);
  ...
  if (!VALID_TEXT_SCALES.has(state.textScale)) state.textScale = 'normal';
  ...
  const dom = {
    ...
    btnTrackerSettings: $('btnTrackerSettings'),
    btnCloseDashboardControls: $('btnCloseDashboardControls'),
  ...
  function setDashboardControlsOpen(open, { focus = false } = {}) {
    const next = Boolean(open);
    state.dashboardControlsOpen = next;
    dom.dashboardControls.hidden = !next;
    dom.btnSettings.setAttribute('aria-expanded', next ? 'true' : 'false');
    dom.btnSettings.setAttribute('aria-label', next ? 'Hide dashboard controls' : 'Show additional dashboard controls');
    dom.btnSettings.title = next ? 'Hide additional dashboard controls' : 'Show additional dashboard controls';

    if (next) {
      requestAnimationFrame(() => {
        dom.dashboardControls.classList.add('is-open');
        if (focus) {
          const target = dom.dashboardControls.querySelector('button:not([hidden])');
          if (target) target.focus();
        }
      });
    } else {
      dom.dashboardControls.classList.remove('is-open');
      if (focus) dom.btnSettings.focus();
    }
  }

  function toggleDashboardControls() {
    setDashboardControlsOpen(!state.dashboardControlsOpen, { focus: true });
  }
  ...
  function applyTextScale(scale) {
    const normalized = VALID_TEXT_SCALES.has(scale) ? scale : 'normal';
    state.textScale = normalized;
    document.documentElement.setAttribute('data-text-scale', normalized);
    const large = normalized === 'large';
    dom.btnTextSize.setAttribute('aria-pressed', large ? 'true' : 'false');
    dom.btnTextSize.textContent = large ? 'A−' : 'A+';
    dom.btnTextSize.title = large ? 'Use normal text size' : 'Increase text size';
    dom.btnTextSize.setAttribute('aria-label', large ? 'Use normal text size' : 'Increase text size');
  }

  function toggleTextScale() {
    const next = state.textScale === 'large' ? 'normal' : 'large';
    applyTextScale(next);
    settings.set('textScale', next);
  }
  ...
  function applyContrast(enabled) {
    ...
    dom.btnContrast.setAttribute('aria-pressed', state.highContrast ? 'true' : 'false');
    dom.btnContrast.textContent = state.highContrast ? '◑' : '◐';
    dom.btnContrast.title = state.highContrast ? 'Use standard contrast' : 'Toggle high contrast';
    dom.btnContrast.setAttribute('aria-label', state.highContrast ? 'Use standard contrast' : 'Toggle high contrast');
  }

  function toggleContrast() {
    applyContrast(!state.highContrast);
    settings.set('highContrast', state.highContrast);
  }
  ...
  function renderFocusControls() {
    const maximized = state.focusList;
    const uiHidden = state.focusList && state.focusUiHidden;

    dom.btnFocusList.textContent = maximized ? 'Exit maximize' : 'Maximize';
    dom.btnFocusList.setAttribute('aria-pressed', maximized ? 'true' : 'false');
    dom.btnFocusList.title = maximized ? 'Exit maximize account list' : 'Maximize account list';
    dom.btnFocusList.setAttribute('aria-label', maximized ? 'Exit maximize account list' : 'Maximize account list');

    dom.btnHideUi.hidden = !maximized;
    dom.btnHideUi.textContent = uiHidden ? 'Show UI' : 'Hide UI';
    dom.btnHideUi.setAttribute('aria-pressed', uiHidden ? 'true' : 'false');
    dom.btnHideUi.title = uiHidden ? 'Show surrounding dashboard UI' : 'Hide surrounding dashboard UI';
    dom.btnHideUi.setAttribute('aria-label', uiHidden ? 'Show surrounding dashboard UI' : 'Hide surrounding dashboard UI');
    dom.btnShowUi.hidden = !uiHidden;
  }

  function applyFocusLayout() {
    document.body.classList.toggle('focus-list', state.focusList);
    document.body.classList.toggle('focus-list-ui-hidden', state.focusList && state.focusUiHidden);
    renderFocusControls();
  }

  function toggleFocusList(force) {
    const next = typeof force === 'boolean' ? force : !state.focusList;
    const scrollTop = dom.listViewport.scrollTop;
    state.focusList = next;
    if (!state.focusList) state.focusUiHidden = false;

    applyFocusLayout();
    requestAnimationFrame(() => {
      dom.listViewport.scrollTop = scrollTop;
      if (state.focusList) {
        try { dom.listViewport.focus({ preventScroll: true }); } catch (_err) {}
      }
    });
  }

  function toggleFocusUi(force) {
    if (!state.focusList) return;
    const next = typeof force === 'boolean' ? force : !state.focusUiHidden;
    const scrollTop = dom.listViewport.scrollTop;
    state.focusUiHidden = next;
    applyFocusLayout();
    requestAnimationFrame(() => { dom.listViewport.scrollTop = scrollTop; });
  }
  ...
  function wireEvents() {
    ...
    dom.btnSettings.addEventListener('click', toggleDashboardControls);
    dom.btnCloseDashboardControls.addEventListener('click', () => setDashboardControlsOpen(false, { focus: true }));
    dom.btnTrackerSettings.addEventListener('click', openSettingsModal);
    dom.btnAboutInline.addEventListener('click', openAboutModal);
    dom.btnTheme.addEventListener('click', toggleTheme);
    dom.btnTextSize.addEventListener('click', toggleTextScale);
    ...
    dom.btnHideUi.addEventListener('click', () => toggleFocusUi());
    dom.btnShowUi.addEventListener('click', () => toggleFocusUi(false));
